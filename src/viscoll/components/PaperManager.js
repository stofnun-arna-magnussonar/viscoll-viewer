...
PaperManager.prototype = {
	...
	draw: function() {
		...
		for (let i in this.groupIDs) {
			...
			if (group.nestLevel === 1) {
				...
				console.log(group)
				currentY = this.calculateYs(group.memberOrders, currentY, this.spacing);

				if (group.memberOrders.length===0) {
	...
	activateTacketTool: function(groupID, type="tacketed") {
		...
		this.tool.onMouseDrag = (event) => {
			...
			targetGroup.group.memberOrders.forEach((memberID)=> {
	...
	getYOfFirstMember: function(groupID) {
		...
		let firstMemberID = group.memberOrders[0];
	...
	getLastMember: function(groupID) {
		 let lastmemberOrders =  this.Groups[groupID].memberOrders;
		if (lastmemberOrders.length===0) return null;
		let lastMemberID = lastmemberOrders[lastmemberOrders.length-1];
	...
	calculateYs: function(members, currentY, spacing) {
		...
		members.forEach((memberID, i)=> {
			...
			let notesToShowAbove = memberObject.notes ? memberObject.notes.filter((noteID)=>{return this.Notes[noteID].show}).length : 0;
			...
			if (memberObject.memberType==="Leaf") {
				console.log(this)
				console.log(memberObject)
				console.log(this.Rectos[memberObject.rectoOrder])
				// Find if it has side notes 
				notesToShowAbove += this.Rectos[memberObject.rectoID].notes ? this.Rectos[memberObject.rectoID].notes.filter((noteID)=>{return this.Notes[noteID].show}).length : 0;
				notesToShowBelow += this.Versos[memberObject.versoID].notes ? this.Versos[memberObject.versoID].notes.filter((noteID)=>{return this.Notes[noteID].show}).length : 0;
			...
			if (memberObject.memberType === "Leaf" && getMemberOrder(memberObject, this.Groups, this.groupIDs)===1 && notesToShowAbove>0) {
				...
				if (i > 0 && members[i-1].includes("Group") && this.Groups[members[i-1]].memberOrders.length) {
				...
				if (i > 0 && members[i-1].includes("Group") && this.Groups[members[i-1]].memberOrders.length>0) {
					// Previous sibling is a group with children
					const previousMember = this.getLastMember(members[i-1]);
					currentY = currentY + (previousMember.nestLevel - memberObject.nestLevel + 1)*spacing;
				} else if (i > 0 && members[i-1].includes("Group")&& this.Groups[members[i-1]].memberOrders.length===0) {
				...
				// Recursify!!!
				currentY = this.calculateYs(memberObject.memberOrders, currentY, spacing);
			}
		});
		return currentY;
	},
