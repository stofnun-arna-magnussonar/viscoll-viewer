...
PaperLeaf.prototype = {
	...
	draw: function() {
		...
		const leafNotesToShow = this.leaf && this.leaf.notes ? this.leaf.notes.filter((noteID)=>{return this.Notes[noteID].show}).reverse() : [];
		const rectoNotesToShow = this.recto && this.recto.notes ? this.recto.notes.filter((noteID)=>{return this.Notes[noteID].show}).reverse() : [];
		const versoNotesToShow = this.verso && this.verso.notes ? this.verso.notes.filter((noteID)=>{return this.Notes[noteID].show}) : [];
		...
		if (this.leaf.params.attached_above.includes("Partial")) {
		...
		} else if (this.leaf.params.attached_above.includes("Glued")) {
	...
	createAttachments: function() {
		if (this.order>1 && this.leaf.params.attached_above.includes("Glued")) {
			this.createGlue();
		} else if (this.order>1 && this.leaf.params.attached_above==="Other") {
		...
		const versoValues = [];
		 if (this.visibleAttributes.side) {
			...
			if (this.visibleAttributes.side.page_number) {
				if (this.recto && this.recto.page_number) rectoValues.push(this.recto.page_number)
				if (this.verso && this.verso.page_number) versoValues.push(this.verso.page_number)
			}
...
function PaperLeaf(args) {
	...
	// SKOÐA this.groupDirection = args.Groups[this.leaf.parentID].direction;
